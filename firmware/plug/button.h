...
/** 
    ...
    @param super_long_press_time The duration that should be recognized as a super long press, in miliseconds.
    @param callback The callback that is called when an "button" event occurs.
    @return A negative integer if this method fails.
*/
int button_create(uint8_t gpio_num, bool pressed_value, uint16_t long_press_time, uint16_t super_long_press_time, button_callback_fn callback);
