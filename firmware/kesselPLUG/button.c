...
void button_intr_callback(uint8_t gpio) {
    ...
    if (gpio_read(button->gpio_num) == button->pressed_value) {
        ...
        if ((now - button->last_press_time) * portTICK_PERIOD_MS > button->super_long_press_time) {
            button->callback(button->gpio_num, button_event_super_long_press);
	} else if ((now - button->last_press_time) * portTICK_PERIOD_MS > button->long_press_time) {
...
int button_create(const uint8_t gpio_num, bool pressed_value, uint16_t long_press_time, uint16_t super_long_press_time, button_callback_fn callback) {
    ...
    button->super_long_press_time = super_long_press_time;
